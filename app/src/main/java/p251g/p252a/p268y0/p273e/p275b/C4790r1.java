package p251g.p252a.p268y0.p273e.p275b;

import java.util.concurrent.TimeUnit;
import java.util.concurrent.atomic.AtomicLong;
import java.util.concurrent.atomic.AtomicReference;
import p251g.p252a.AbstractC4516j0;
import p251g.p252a.AbstractC4519l;
import p251g.p252a.p264u0.InterfaceC4552c;
import p251g.p252a.p265v0.C4562c;
import p251g.p252a.p268y0.p269a.EnumC4592d;
import p251g.p252a.p268y0.p281g.C5129r;
import p251g.p252a.p268y0.p283i.EnumC5160j;
import p251g.p252a.p268y0.p284j.C5164d;
import p286h.p309q2.p311t.C5556m0;
import p324i.p338d.InterfaceC5822c;
import p324i.p338d.InterfaceC5823d;

/* compiled from: FlowableIntervalRange.java */
/* renamed from: g.a.y0.e.b.r1 */
/* loaded from: classes2.dex */
public final class C4790r1 extends AbstractC4519l<Long> {

    /* renamed from: b */
    final AbstractC4516j0 f18351b;

    /* renamed from: c */
    final long f18352c;

    /* renamed from: d */
    final long f18353d;

    /* renamed from: e */
    final long f18354e;

    /* renamed from: f */
    final long f18355f;

    /* renamed from: g */
    final TimeUnit f18356g;

    /* compiled from: FlowableIntervalRange.java */
    /* renamed from: g.a.y0.e.b.r1$a */
    static final class a extends AtomicLong implements InterfaceC5823d, Runnable {
        private static final long serialVersionUID = -2809475196591179431L;
        final InterfaceC5822c<? super Long> actual;
        long count;
        final long end;
        final AtomicReference<InterfaceC4552c> resource = new AtomicReference<>();

        a(InterfaceC5822c<? super Long> interfaceC5822c, long j2, long j3) {
            this.actual = interfaceC5822c;
            this.count = j2;
            this.end = j3;
        }

        @Override // p324i.p338d.InterfaceC5823d
        public void cancel() {
            EnumC4592d.dispose(this.resource);
        }

        @Override // p324i.p338d.InterfaceC5823d
        public void request(long j2) {
            if (EnumC5160j.validate(j2)) {
                C5164d.m18607a(this, j2);
            }
        }

        @Override // java.lang.Runnable
        public void run() {
            if (this.resource.get() != EnumC4592d.DISPOSED) {
                long j2 = get();
                if (j2 == 0) {
                    this.actual.onError(new C4562c("Can't deliver value " + this.count + " due to lack of requests"));
                    EnumC4592d.dispose(this.resource);
                    return;
                }
                long j3 = this.count;
                this.actual.onNext(Long.valueOf(j3));
                if (j3 == this.end) {
                    if (this.resource.get() != EnumC4592d.DISPOSED) {
                        this.actual.onComplete();
                    }
                    EnumC4592d.dispose(this.resource);
                } else {
                    this.count = j3 + 1;
                    if (j2 != C5556m0.f20396b) {
                        decrementAndGet();
                    }
                }
            }
        }

        public void setResource(InterfaceC4552c interfaceC4552c) {
            EnumC4592d.setOnce(this.resource, interfaceC4552c);
        }
    }

    public C4790r1(long j2, long j3, long j4, long j5, TimeUnit timeUnit, AbstractC4516j0 abstractC4516j0) {
        this.f18354e = j4;
        this.f18355f = j5;
        this.f18356g = timeUnit;
        this.f18351b = abstractC4516j0;
        this.f18352c = j2;
        this.f18353d = j3;
    }

    @Override // p251g.p252a.AbstractC4519l
    /* renamed from: d */
    public void mo17216d(InterfaceC5822c<? super Long> interfaceC5822c) {
        a aVar = new a(interfaceC5822c, this.f18352c, this.f18353d);
        interfaceC5822c.onSubscribe(aVar);
        AbstractC4516j0 abstractC4516j0 = this.f18351b;
        if (!(abstractC4516j0 instanceof C5129r)) {
            aVar.setResource(abstractC4516j0.mo17408a(aVar, this.f18354e, this.f18355f, this.f18356g));
            return;
        }
        AbstractC4516j0.c mo17277a = abstractC4516j0.mo17277a();
        aVar.setResource(mo17277a);
        mo17277a.mo17412a(aVar, this.f18354e, this.f18355f, this.f18356g);
    }
}
