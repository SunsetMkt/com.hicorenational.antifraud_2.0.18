...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j2) {
            ...
            if (j3 < 0 || j2 < j3) {
                return AutoScrollHelper.constrain((j2 - this.mStartTime) / this.mRampUpDuration, 0.0f, 1.0f) * 0.5f;
            }
            long j4 = j2 - j3;
            float f2 = this.mStopValue;
            return (1.0f - f2) + (f2 * AutoScrollHelper.constrain(j4 / this.mEffectiveRampDown, 0.0f, 1.0f));
        }
