...
public class d {
    ...
    public d(boolean z, String str) {
        ...
        if (str != null) {
            if (!z) {
                String[] split = str.split(f10004b);
                int length = split.length;
                while (i2 < length) {
                    this.f10007d.add(split[i2]);
                    i2++;
                }
                return;
            }
            try {
                ...
                while (i2 < decode.length / 4) {
                    int i3 = i2 * 4;
                    this.f10007d.add(Integer.valueOf(((decode[i3 + 0] & f1.f16099c) << 24) + ((decode[i3 + 1] & f1.f16099c) << 16) + ((decode[i3 + 2] & f1.f16099c) << 8) + (decode[i3 + 3] & f1.f16099c)));
                    i2++;
                }
    ...
    private Integer c(String str) {
        try {
            ...
            byte[] digest = this.f10006c.digest();
            return Integer.valueOf(((digest[0] & f1.f16099c) << 24) + ((digest[1] & f1.f16099c) << 16) + ((digest[2] & f1.f16099c) << 8) + (digest[3] & f1.f16099c));
        } catch (Exception e2) {
            e2.printStackTrace();
            return null;
        }
