...
public class h<T> implements n<T> {
    ...
    private final Collection<? extends n<T>> f2716c;
    ...
    public h(@NonNull n<T>... nVarArr) {
        ...
        this.f2716c = Arrays.asList(nVarArr);
    }

    @Override // com.bumptech.glide.load.n
    @NonNull
    public v<T> a(@NonNull Context context, @NonNull v<T> vVar, int i2, int i3) {
        Iterator<? extends n<T>> it = this.f2716c.iterator();
        v<T> vVar2 = vVar;
        while (it.hasNext()) {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof h) {
            return this.f2716c.equals(((h) obj).f2716c);
        }
        return false;
    }
