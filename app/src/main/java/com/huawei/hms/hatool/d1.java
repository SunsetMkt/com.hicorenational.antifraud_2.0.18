...
public class d1 implements g {
    ...
    private void a() {
        d.a(q0.i(), "backup_event", n1.a(this.a, this.f4715c, this.f4714b));
    }

    @Override // java.lang.Runnable
    public void run() {
        List<b1> list = this.f4716d;
        if (list == null || list.size() == 0) {
            v.d("hmsSdk", "failed events is empty");
            return;
        }
        if (c0.a(q0.i(), "cached_v2_1", q0.k() * 1048576)) {
            v.e("hmsSdk", "The cacheFile is full,Can not writing data! reqID:" + this.f4714b);
            return;
        }
        String strA = n1.a(this.a, this.f4715c);
        List<b1> list2 = c1.b(q0.i(), "cached_v2_1", strA).get(strA);
        if (list2 != null && list2.size() != 0) {
            this.f4716d.addAll(list2);
        }
        JSONArray jSONArray = new JSONArray();
        Iterator<b1> it = this.f4716d.iterator();
        while (it.hasNext()) {
            try {
        ...
        v.d("hmsSdk", "data send failed, write to cache file...reqID:" + this.f4714b);
