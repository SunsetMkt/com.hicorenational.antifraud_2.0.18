...
public class BaseResolutionAdapter implements IBridgeActivityDelegate {
    ...
    private void a(long j2) {
        ...
        map.put(CommonCode.MapKey.RESOLUTION_FLAG, this.f4451c + Constants.ACCEPT_TIME_SEPARATOR_SERVER + j2);
        HiAnalyticsUtils.getInstance().onEvent(activityC, HiAnalyticsConstant.HMS_SDK_BASE_START_RESOLUTION, map);
        HMSLog.e("BaseResolutionAdapter", "check resolution flag failed, transactionId: " + this.f4450b + ", carriedTimeStamp: " + this.f4451c + ", savedTimeStamp: " + j2);
    }
    ...
    private void d() {
        SystemManager.getInstance().notifyResolutionResult(null, this.f4450b);
        b();
    }
    ...
    public void onBridgeActivityCreate(Activity activity) {
        ...
        try {
            ...
            this.f4450b = intent.getStringExtra(CommonCode.MapKey.TRANSACTION_ID);
            this.f4451c = intent.getLongExtra(CommonCode.MapKey.RESOLUTION_FLAG, 0L);
        } catch (Throwable th) {
        ...
        if (this.f4450b != null && Build.VERSION.SDK_INT >= 29) {
            HMSLog.i("BaseResolutionAdapter", "remove apk resolution failed task.");
            ApkResolutionFailedManager.getInstance().removeTask(this.f4450b);
        }
        if (extras == null) {
            d();
            return;
    ...
    public boolean onBridgeActivityResult(int i2, int i3, Intent intent) {
        ...
        if (i3 == -1 || intent.hasExtra(KpmsConstant.KIT_UPDATE_RESULT) || intent.hasExtra(CommonCode.MapKey.PRIVACY_STATEMENT_CONFIRM_RESULT)) {
            SystemManager.getInstance().notifyResolutionResult(intent, this.f4450b);
        } else {
            SystemManager.getInstance().notifyResolutionResult(null, this.f4450b);
        }
        b();
    ...
    private boolean a() {
        long resolutionFlag = ResolutionFlagUtil.getInstance().getResolutionFlag(this.f4450b);
        ResolutionFlagUtil.getInstance().removeResolutionFlag(this.f4450b);
        if (resolutionFlag != 0 && resolutionFlag == this.f4451c) {
            return true;
        }
        a(resolutionFlag);
        return false;
