...
public class HmsMessaging {
    ...
    private static final Pattern f4849c = Pattern.compile("[\\u4e00-\\u9fa5\\w-_.~%]{1,900}");
    private Context a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    private HuaweiApi<Api.ApiOptions.NoOptions> f4850b;
    ...
    private HmsMessaging(Context context) {
        ...
        if (context instanceof Activity) {
            this.f4850b = new HuaweiApi<>((Activity) context, (Api<Api.ApiOptions>) api, (Api.ApiOptions) null, (AbstractClientBuilder) new PushClientBuilder());
        } else {
            this.f4850b = new HuaweiApi<>(context, (Api<Api.ApiOptions>) api, (Api.ApiOptions) null, new PushClientBuilder());
        }
        this.f4850b.setKitSdkVersion(61200300);
    }

    private Task<Void> a(String str, String str2) {
        String strReportEntry = PushBiUtil.reportEntry(this.a, PushNaming.SUBSCRIBE);
        if (str == null || !f4849c.matcher(str).matches()) {
        ...
        try {
            ...
            return d.b() ? this.f4850b.doWrite(new BaseVoidTask(PushNaming.SUBSCRIBE, JsonUtil.createJsonString(subscribeReq), strReportEntry)) : this.f4850b.doWrite(new SubscribeTask(PushNaming.SUBSCRIBE, JsonUtil.createJsonString(subscribeReq), strReportEntry));
        } catch (ApiException e2) {
            TaskCompletionSource taskCompletionSource = new TaskCompletionSource();
            taskCompletionSource.setException(e2);
            PushBiUtil.reportExit(this.a, PushNaming.SUBSCRIBE, strReportEntry, e2.getStatusCode());
            return taskCompletionSource.getTask();
        } catch (Exception unused) {
            TaskCompletionSource taskCompletionSource2 = new TaskCompletionSource();
            taskCompletionSource2.setException(ErrorEnum.ERROR_INTERNAL_ERROR.toApiException());
            PushBiUtil.reportExit(this.a, PushNaming.SUBSCRIBE, strReportEntry, ErrorEnum.ERROR_INTERNAL_ERROR);
            ...
            if (!TextUtils.isEmpty(remoteMessage.getTo())) {
                if (!TextUtils.isEmpty(remoteMessage.getMessageId())) {
                    if (!TextUtils.isEmpty(remoteMessage.getData())) {
                        ...
                        if (d.b()) {
                            this.f4850b.doWrite(new BaseVoidTask(PushNaming.UPSEND_MSG, JsonUtil.createJsonString(upSendMsgReq), strReportEntry));
                            return;
                        } else {
    ...
    private Task<Void> a(boolean z) {
        ...
        if (d.d(this.a) && !d.b()) {
            if (HwBuildEx.VERSION.EMUI_SDK_INT < 12) {
                ...
                PushBiUtil.reportExit(this.a, PushNaming.SET_NOTIFY_FLAG, strReportEntry, ErrorEnum.ERROR_OPERATION_NOT_SUPPORTED);
                return taskCompletionSource.getTask();
            }
            ...
            return Tasks.callInBackground(new IntentCallable(this.a, intent2, strReportEntry));
        }
        ...
        return this.f4850b.doWrite(new BaseVoidTask(PushNaming.SET_NOTIFY_FLAG, JsonUtil.createJsonString(enableNotifyReq), strReportEntry));
    }

    private void a(UpSendMsgReq upSendMsgReq, String str) {
        upSendMsgReq.setToken(BaseUtils.getLocalToken(this.a, null));
        try {
            this.f4850b.doWrite(new SendUpStreamTask(PushNaming.UPSEND_MSG, JsonUtil.createJsonString(upSendMsgReq), str, upSendMsgReq.getPackageName(), upSendMsgReq.getMessageId()));
        } catch (Exception e2) {
