package h.a.y0.e.c;

/* JADX INFO: compiled from: MaybeDoAfterSuccess.java */
/* JADX INFO: loaded from: classes2.dex */
@h.a.t0.e
public final class q<T> extends h.a.y0.e.c.a<T, T> {

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    final h.a.x0.g<? super T> f11174b;

    /* JADX INFO: compiled from: MaybeDoAfterSuccess.java */
    static final class a<T> implements h.a.v<T>, h.a.u0.c {
        final h.a.v<? super T> a;

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        final h.a.x0.g<? super T> f11175b;

        /* JADX INFO: renamed from: c, reason: collision with root package name */
        h.a.u0.c f11176c;

        a(h.a.v<? super T> vVar, h.a.x0.g<? super T> gVar) {
            this.a = vVar;
            this.f11175b = gVar;
        }

        @Override // h.a.u0.c
        public void dispose() {
            this.f11176c.dispose();
        }

        @Override // h.a.u0.c
        public boolean isDisposed() {
            return this.f11176c.isDisposed();
        }

        @Override // h.a.v
        public void onComplete() {
            this.a.onComplete();
        }

        @Override // h.a.v
        public void onError(Throwable th) {
            this.a.onError(th);
        }

        @Override // h.a.v
        public void onSubscribe(h.a.u0.c cVar) {
            if (h.a.y0.a.d.validate(this.f11176c, cVar)) {
                this.f11176c = cVar;
                this.a.onSubscribe(this);
            }
        }

        @Override // h.a.v, h.a.n0
        public void onSuccess(T t) {
            this.a.onSuccess(t);
            try {
                this.f11175b.accept(t);
            } catch (Throwable th) {
                h.a.v0.b.b(th);
                h.a.c1.a.b(th);
            }
        }
    }

    public q(h.a.y<T> yVar, h.a.x0.g<? super T> gVar) {
        super(yVar);
        this.f11174b = gVar;
    }

    @Override // h.a.s
    protected void b(h.a.v<? super T> vVar) {
        this.a.a(new a(vVar, this.f11174b));
    }
}
