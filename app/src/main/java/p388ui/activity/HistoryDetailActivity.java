...
import p388ui.presenter.HistoryDetaPresenter;
...
public class HistoryDetailActivity extends BaseActivity implements HistoryDetailCallView {
    ...
    @BindView(C2113R.id.casecategory)
    TextView mCaseTegory;

    @BindView(C2113R.id.tv_case_type)
    TextView mCaseTip;
    ...
    @BindView(C2113R.id.ll_report_progress)
    LinearLayout mLlReportProgress;

    @BindView(C2113R.id.ll_report_view)
    View mLlReportView;
    private HistoryDetaPresenter mPresenter;

    @BindView(C2113R.id.tv_case_num)
    TextView mTvCaseNum;

    @BindView(C2113R.id.tv_report_name)
    TextView mTvReportInfo;

    @BindView(C2113R.id.tv_report_num)
    TextView mTvReportNum;

    @BindView(C2113R.id.report_time)
    TextView mTvReportTime;

    @BindView(C2113R.id.tv_self)
    TextView mTvSelf;
    ...
    private void DefineViewData(HistoryDetailInfo historyDetailInfo) {
        ...
        for (int i2 = 0; i2 < historyDetailInfo.replys.size(); i2++) {
            View inflate = LayoutInflater.from(this.mActivity).inflate(C2113R.layout.report_reply_item, (ViewGroup) null, false);
            ImageView imageView = (ImageView) inflate.findViewById(C2113R.id.iv_reply_img);
            TextView textView = (TextView) inflate.findViewById(C2113R.id.stp_line);
            TextView textView2 = (TextView) inflate.findViewById(C2113R.id.tv_title);
            TextView textView3 = (TextView) inflate.findViewById(C2113R.id.tv_time);
            TextView textView4 = (TextView) inflate.findViewById(C2113R.id.tv_content);
            TextView textView5 = (TextView) inflate.findViewById(C2113R.id.tv_line);
            ...
            if (i3 == 3) {
                ...
                textView3.setText(replay.replyTime);
                textView4.setText(TextUtils.isEmpty(replay.description) ? "" : replay.description);
                textView5.setBackgroundResource(C2113R.color.transparent);
            } else {
            ...
            this.mLlReportProgress.addView(inflate);
        }
    }

    private void getIntentData() {
        try {
            this.pageType = getIntent().getIntExtra(C7292k1.f25337C, 0);
            this.rowsBean = (HistoryListInfo.RowsBean) getIntent().getSerializableExtra(C7292k1.f25335B);
            this.info = this.rowsBean.getInfo();
            this.mPresenter.getDetail(this.info.getId());
            int parseInt = Integer.parseInt(this.rowsBean.getEvidenceCount());
            this.mTvReportNum.setText("（" + parseInt + "项举报内容）");
    ...
    @Override // p388ui.callview.HistoryDetailCallView
    public void onSuccessRequest(HistoryDetailInfo historyDetailInfo) {
        ...
        if (list3 != null && list3.size() > 0) {
            ...
            for (int i4 = 0; i4 < historyDetailInfo.apps.size(); i4++) {
                ...
                detailBean3.setOs(historyDetailInfo.f21434os);
    ...
    public int setLayoutView() {
        return C4440a.m16409k() ? C2113R.layout.activity_history_detail_local : C2113R.layout.activity_history_detail;
    }
}
