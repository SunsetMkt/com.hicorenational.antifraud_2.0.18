...
class h implements EventCb {
    ...
    public void onEvent(Session session, int i2, anet.channel.entity.b bVar) {
        ...
        HttpUrl parse = HttpUrl.parse(session.getHost() + this.f1725d.f2021c);
        if (parse == null) {
            return;
        }
        this.f1726e.request(new Request.Builder().setUrl(parse).setReadTimeout(this.f1725d.f2020b.f1992d).setRedirectEnable(false).setSeq(this.f1724c).build(), new i(this));
    }
}
