...
public class a implements IAppMonitor {
    ...
    private static Map<Class<?>, List<Field>> f1386b = new ConcurrentHashMap();

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    private static Map<Class<?>, List<Field>> f1387c = new ConcurrentHashMap();

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    private static Map<Field, String> f1388d = new ConcurrentHashMap();

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    private static Random f1389e = new Random();

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    private static Set<Class<?>> f1390f = Collections.newSetFromMap(new ConcurrentHashMap());
        ...
        if (cls != null) {
            if (a) {
                try {
                    ...
                    f1386b.put(cls, arrayList);
                    f1387c.put(cls, arrayList2);
    ...
    public void commitStat(StatObject statObject) {
        ...
        if (statObject.beforeCommit()) {
            if (monitor.monitorPoint().equals("network")) {
                ...
                if (iB != 10000 && f1389e.nextInt(10000) >= iB) {
                    return;
                }
            }
            try {
                ...
                List<Field> list = f1386b.get(cls);
                HashMap map = ALog.isPrintLog(1) ? new HashMap() : null;
                if (list != null) {
                    for (Field field : list) {
                        Object obj = field.get(statObject);
                        dimensionValueSetCreate.setValue(f1388d.get(field), obj == null ? "" : obj.toString());
                    }
                    for (Field field2 : f1387c.get(cls)) {
                        Double dValueOf = Double.valueOf(field2.getDouble(statObject));
                        measureValueSetCreate.setValue(f1388d.get(field2), dValueOf.doubleValue());
                        if (map != null) {
                            map.put(f1388d.get(field2), dValueOf);
                        }
                    }
                }
